...
export function csrfProtection(req: Request, res: Response, next: NextFunction) {
  ...
  if (!expected || !supplied) return res.status(403).json({ error: { code: "FORBIDDEN", message: "Token CSRF ausente" } });
  const a = Buffer.from(expected);
  const b = Buffer.from(supplied);
  if (a.length !== b.length || !timingSafeEqual(a, b)) return res.status(403).json({ error: { code: "FORBIDDEN", message: "Token CSRF inválido" } });
  next();
}
