 ...
 * 
 * USO:
 *   node --env-file=.env script/run-seed.mjs              # Insere dados (não sobrescreve existentes)
 *   node --env-file=.env script/run-seed.mjs --if-empty   # Só insere se banco estiver vazio (deploy)
 *   node --env-file=.env script/run-seed.mjs --fresh      # Limpa TODAS as tabelas e insere do zero
 * 
 * IMPORTANTE:
